...
//
//	POST /workspace/reset  — ClearRuntime (narrow)
...
// scripts cannot POST to these endpoints without the broker token — these
// operations are strictly more destructive than /config or /company, which
// are already auth-gated. Pass a passthrough (func(h) http.HandlerFunc { return h })
// only in tests.
