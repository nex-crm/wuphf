...
func planModeDirective(task teamTask) string {
	...
	return "[PLAN MODE] This task is in structured planning and your runtime is read-only — do NOT change the repo, run build/deploy steps, create sub-tasks, or take external actions this turn (the sandbox will block them anyway). Plan first:\n" +
		"1. Read only what you need to understand the work; do not do the work.\n" +
		"2. ASK before you assume. If the outcome, scope, success criteria, owner, or access is genuinely unclear, call human_interview ONCE with the open questions batched into a single set — never guess, never one interview per question. Skip the interview entirely if nothing is a genuine gap.\n" +
		"3. Produce a tight PLAN: the goal, the concrete step-by-step approach, the SPECIFIC sub-tasks you will create (each a distinct, non-overlapping slice — no sub-task that merely restates the goal or duplicates a sibling), acceptance criteria, and risks/open questions. Present it as your final answer (in plan mode this goes through the plan/ExitPlanMode surface). If your runtime still allows writes, also save it to your notebook with notebook_write (path like " + notebookPath + ") — a draft for you and the human, NOT promoted to the team wiki unless @librarian decides it is worth it.\n" +
		"4. Then STOP — the plan is surfaced to the human for approval automatically. Execution and sub-task creation start ONLY after the human approves the plan; you will be re-notified to begin then. Do NOT try to start implementing or decompose into sub-tasks in this turn.\n" +
		"---\n"
}
