...
// resolvePermissionFlags returns the Claude Code permission flags for an
// agent's turn. Every turn runs in bypass mode — the team is autonomous.
func (l *Launcher) resolvePermissionFlags() string {
...
func (l *Launcher) claudeCommand(slug, systemPrompt string) (string, error) {
	...
	permFlags := l.resolvePermissionFlags()
