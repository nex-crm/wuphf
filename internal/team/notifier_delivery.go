...
func (l *Launcher) slackChannelConventionNote(channel string) string {
	...
	return "\n---\nSLACK CHANNEL CONVENTIONS (this channel is bridged to a real Slack workspace with real people and external agents): " +
		"(1) NEVER @-tag an agent or person unless you need them to act or respond right now: a tag pings them and they WILL respond. For FYI/status references use the plain name with no @. " +
		"(2) To hand work to an external agent, give it its OWN subtask (team_task action=create with that agent as owner). The office addresses the agent inside that subtask's thread with the full context, and that is where it works. In the parent task and the main channel, refer to the agent by plain name with no @ so you do not pull it into the wrong thread. Do NOT @-tag the assignee in the parent when you delegate. " +
		...
		"(4) MAIN CHANNEL passivity: if a message in the main channel (not a task thread) needs no action from you, post NOTHING. Never post acknowledgement-only replies (\"noted\", \"acknowledged\", \"no action needed\"): real people read this channel, and repeating the same status is spam. Summarize once when the situation changes, not once per incoming message. " +
		...
		"(6) INSIDE A TASK THREAD you are NOT passive. Watch what the assignee and others post there and reply whenever a response moves the work forward: answer a question, unblock, course-correct, or confirm a deliverable. The 'post nothing unless tagged' rule in (4) is for the MAIN channel only. An assignee posting progress in its task thread does not need to tag you for you to engage."
}
