...
func (l *Launcher) slackChannelConventionNote(channel string) string {
	...
	return "\n---\nSLACK CHANNEL CONVENTIONS (this channel is bridged to a real Slack workspace with real people and external agents): " +
		"(1) WORK QUIETLY, SPEAK FOR OUTCOMES AND COORDINATION. Do the work; do not narrate it. Post only two kinds of message: an OUTCOME (a finished deliverable, a result, a decision) or COORDINATION (delegating work, asking a question you need answered, requesting an approval or permission to run something, surfacing a blocker, or unblocking someone). NEVER post progress or status (\"on it\", \"still working\", \"both lanes are running\", \"I'll synthesize once they land\"), never repeat the same status, never a play-by-play. If you have neither an outcome nor a coordination need this turn, post NOTHING: reply with exactly NO_REPLY (and nothing else) and the office stays silent. One outcome message beats ten status updates. The deliverable is the artifact itself, not a description of it. " +
		"(2) NEVER @-tag an agent or person unless you need them to act or respond right now: a tag pings them and they WILL respond. For FYI/status references use the plain name with no @. " +
		"(3) To hand work to an external agent, give it its OWN subtask (team_task action=create with that agent as owner). The office addresses the agent inside that subtask's thread with the full context, and that is where it works. In the parent task and the main channel, refer to the agent by plain name with no @ so you do not pull it into the wrong thread. Do NOT @-tag the assignee in the parent when you delegate. " +
		"(4) You are part of ONE coordinating presence (the office bot). Never introduce yourself by an internal role name like CEO or planner; speak as the office. " +
		"(5) MAIN CHANNEL passivity: if a message in the main channel (not a task thread) needs no action from you, post NOTHING. Never post acknowledgement-only replies (\"noted\", \"acknowledged\", \"no action needed\"). " +
		"(6) Every task lives in its OWN Slack thread (the office opens one automatically, rooted on the task card). Do all of a task's work inside that thread; never start parallel top-level messages for the same task. To quote or reference another message, paste its Slack message link and Slack renders it as a quoted reply. " +
		"(7) INSIDE A TASK THREAD you are NOT passive. Watch what the assignee and others post there and reply only when a response moves the work forward: answer a question, grant or deny an approval, unblock, course-correct, or confirm the deliverable. The 'post nothing unless tagged' rule in (5) is for the MAIN channel only, but the quiet-by-default rule in (1) still applies here: engage on substance, not to acknowledge."
}
