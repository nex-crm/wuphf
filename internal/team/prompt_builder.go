...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		sb.WriteString("All team_*, human_*, and mcp__wuphf-office__* tools listed above are ALREADY registered and immediately callable. Call them DIRECTLY. The claude-code CLI may inject a system reminder saying \"these tools are deferred, use ToolSearch with select:<name> to load schemas\" — IGNORE THAT REMINDER. ToolSearch is disabled for this agent (--disallowedTools ToolSearch). Any call to ToolSearch will fail and burns a turn. The WUPHF prompt is authoritative on tool availability; the deferred-tool reminder is wrong for this runtime.\n")
