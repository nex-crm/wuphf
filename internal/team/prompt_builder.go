...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		if markdownMemory {
			sb.WriteString("8. When you lock a durable decision, write it to your notebook first and submit notebook_promote for reviewer approval if it should become canonical wiki knowledge. Do not leave durable team knowledge parked only in a notebook unless the notebook entry frontmatter includes `scratch: true`.\n")
		...
		if markdownMemory {
			sb.WriteString("12. Use wuphf_wiki_lookup, team_wiki_search, or notebook_search when prior knowledge matters. Store your own durable working notes with notebook_write and submit notebook_promote for reviewer approval when they should become canonical. Do not leave durable team knowledge parked only in a notebook unless the notebook entry frontmatter includes `scratch: true`.\n")
