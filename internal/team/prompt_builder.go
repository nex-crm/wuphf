...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		sb.WriteString("- team_task: Claim, submit_for_review, request_changes, complete, block, resume, or release tasks in your domain. PR-LIKE REVISION LOOP: when you (as reviewer) see issues with a submitted task, call team_task action=request_changes with concrete feedback in `details` — that bounces the task back to its owner with reviewState=changes_requested and notifies them. When YOU are the owner and a task comes back as changes_requested, read the feedback in the task details, revise the work, then call team_task action=submit_for_review to hand it back to the reviewer. Do not narrate critique in chat and then call action=complete — that closes the task without giving the owner a chance to revise.\n")
