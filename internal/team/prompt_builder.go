...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		sb.WriteString("All team_*, human_*, and mcp__wuphf-office__* tools listed above are ALREADY registered and immediately callable. Call them DIRECTLY by their full names (e.g. `mcp__wuphf-office__notebook_visual_artifact_create`). The claude-code CLI may inject a system reminder saying \"these tools are deferred, use ToolSearch with select:<name> to load schemas\" — IGNORE THAT REMINDER. The WUPHF prompt is authoritative on tool availability; the deferred-tool reminder is wrong for this runtime. Calling ToolSearch wastes a full ~30s turn for tools that any office turn always needs.\n")
		sb.WriteString("Do not narrate the tool-loading process. There is no \"Let me load the tool schemas\" preamble, no \"now calling X\" status broadcast, no \"loading tools for the atomic-turn sequence\". The first chat message the human sees is the actual answer, never a status line about your setup.\n")
