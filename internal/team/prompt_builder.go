...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		if markdownMemory {
			sb.WriteString("8. When you lock a durable decision, write it to your notebook first and submit notebook_promote if it should become canonical wiki knowledge. Do not leave durable team knowledge parked only in a notebook unless you explicitly mark it as scratch.\n")
		...
		if markdownMemory {
			sb.WriteString("12. Use wuphf_wiki_lookup, team_wiki_search, or notebook_search when prior knowledge matters. Store your own durable working notes with notebook_write and submit notebook_promote when they should become canonical. Do not leave durable team knowledge parked only in a notebook unless you explicitly mark it as scratch.\n")
...
func markdownKnowledgeToolBlock() string {
	return "- notebook_write: Save your own working notes, rough observations, draft decisions, draft playbooks, and task learnings at agents/{my_slug}/notebook/{date-or-topic}.md. This is the default write path for agent-authored knowledge.\n" +
		...
		"- notebook_promote: Submit a durable notebook entry for reviewer approval into the team wiki. Use this when the team should rely on the note as canonical knowledge. Do not bypass notebook_promote for new agent-authored knowledge.\n" +
		...
		"- team_wiki_write: Direct canonical wiki writes only when the human explicitly asked you to write the article, playbook, or canonical page to the wiki. Pass human_request with the exact human instruction. Do not use this for agent-authored working notes, observations, or proposed knowledge; those start in notebook_write and move through notebook_promote review.\n" +
