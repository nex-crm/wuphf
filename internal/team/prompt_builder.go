...
func markdownKnowledgeToolBlock() string {
	return "- notebook_write: Save your own working notes, rough observations, draft decisions, draft playbooks, and task learnings at agents/{my_slug}/notebook/{date-or-topic}.md. This is the default write path for agent-authored knowledge.\n" +
		...
		"- For tasks with memory_policy=required, pass task_id to wiki/notebook search, notebook_write, and notebook_promote. If the notebook entry should not be promoted, call team_task action=memory with promotion_decision=skipped, notebook_path, and promotion_reason before completing the task.\n"
}
