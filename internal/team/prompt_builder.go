...
func (p *promptBuilder) Build(slug string) string {
	...
	if p.isOneOnOne() {
		...
		if markdownMemory {
			sb.WriteString("Markdown wiki memory is active in this 1:1. When the work would be clearer as a diagram, mockup, report, comparison grid, code explainer, PR review, or interactive tuning surface, build a self-contained HTML article with notebook_visual_artifact_create — the HTML article IS the deliverable, so leave source_path empty (see the HTML ARTICLE RULE below). Keep the HTML self-contained and include visual-artifact:ra_... on its own line when you reference it in chat.\n\n")
