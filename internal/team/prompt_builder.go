...
type promptBuilder struct {
	...
	// skillCatalog renders the per-agent skill catalog section (PR 7).
	// Returns "" when the broker has no visible skills for slug.
	skillCatalog func(slug string) string
...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		if p.skillCatalog != nil {
			sb.WriteString(p.skillCatalog(slug))
		}
