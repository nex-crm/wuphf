...
// recurrenceFloor is the recurrence count at which a shape surfaces even with
// no recognized outcome step. Below it, a single-or-few-shot shape must end in
// an outcome to surface — that is what keeps opaque read-only loops from
// nagging while still catching "you just did this whole thing once" workflows.
...
func DetectWorkflows(manifests []TurnManifest, opts DetectOptions) []DetectionCandidate {
	...
	for _, key := range clusterOrder {
		...
		if !hasOutcome && count < recurrenceFloor {
			continue
		}
