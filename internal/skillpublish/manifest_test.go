...
func TestHubURL_ClaudeMarketplace(t *testing.T) {
	t.Parallel()
	got, err := HubURL("claude-marketplace", "summarize-docs")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	want := "https://raw.githubusercontent.com/claude-marketplace/skills/main/skills/summarize-docs/SKILL.md"
	if got != want {
		t.Fatalf("HubURL marketplace:\n got: %s\nwant: %s", got, want)
	}
}
...
func TestHubFilePath(t *testing.T) {
	...
	cases := []struct {
		...
		{"marketplace", "claude-marketplace", "x", "skills/x/SKILL.md"},
...
func TestHubRepo(t *testing.T) {
	...
	cases := []struct {
		...
		{"claude-marketplace", "claude-marketplace/skills"},
