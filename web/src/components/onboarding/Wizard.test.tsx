...
function pressEnterOn(target: EventTarget = window, opts: Partial<KeyboardEventInit> = {}) {
...
describe('Wizard keyboard advancement', () => {
  ...
  it('Enter advances identity once company + description are filled', async () => {
    ...
    // templates headline.
    await waitFor(() => {
      expect(screen.getByText(/What should your office run\?/i)).toBeInTheDocument()
    })
  })
  ...
  it('Enter in the Nex signup email submits the panel instead of advancing the step', async () => {
    ...
    await waitFor(() => {
      expect(postMock).toHaveBeenCalledWith('/nex/register', { email: 'me@example.com' })
    })
