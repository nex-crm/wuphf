...
import { fetchEntityGraphAll, type EntityKind, type GraphAllResponse } from '../../api/entity'
...
const NODE_STYLES: Record<string, { fill: string; stroke: string; icon: string; label: string }> = {
  people: {
    fill: '#EDE9FE',
    stroke: '#7C3AED',
    icon: '👤', // 👤
...
function styleFor(kind: string) {
  return NODE_STYLES[kind] ?? { fill: '#F3F4F6', stroke: '#6B7280', icon: '◆', label: kind }
}
...
export function GraphApp() {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h3 style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>Entity Graph</h3>
          <p style={{ fontSize: 13, color: 'var(--text-tertiary)', margin: '4px 0 0' }}>
            People, companies, and customers the team has written facts about — and how they
            connect.
          </p>
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-tertiary)', fontVariantNumeric: 'tabular-nums' }}>
          {totalNodes} node{totalNodes === 1 ? '' : 's'} · {totalEdges} edge{totalEdges === 1 ? '' : 's'}
        </div>
      </header>
          ...
          <>
            ...
            >
              ...
              {simResult.edges.map((e, i) => {
                ...
                if (!a || !b) return null
                const active =
                  hoveredEdge === i || hoveredNode === a.id || hoveredNode === b.id
                ...
                return (
                  <g key={i} onMouseEnter={() => setHoveredEdge(i)} onMouseLeave={() => setHoveredEdge(null)}>
                    <line
                      ...
                      x2={x2}
                      y2={y2}
                      stroke={active ? 'var(--accent, #612a92)' : 'var(--border-dark, #cfd1d2)'}
                    ...
                    {hoveredEdge === i && (
                      <g>
                        ...
                        >
                          {e.occurrenceCount}× · {e.label ? e.label.slice(0, 12) : 'mention'}
                        </text>
                      </g>
                    )}
                  </g>
                ...
                return (
                  ...
                  >
                    <circle
                      ...
                      filter={active ? 'drop-shadow(0 4px 12px rgba(0,0,0,0.12))' : undefined}
                    />
                    <text
                      y={-2}
                      textAnchor="middle"
...
function EmptyState() {
  return (
    ...
    >
      ...
      <div>
        Record facts about people, companies, or customers (via the MCP surface or
        <code style={{ padding: '0 4px' }}>POST /entity/fact</code>) and they'll appear
        here with every wikilink they mention.
      </div>
    </div>
  )
}
