...
function statusMeta(status: NotebookEntryStatus): { label: string; className: string; aria: string } {
  switch (status) {
    case 'promoted':
      return { label: 'Promoted', className: 'nb-promoted-pill', aria: 'Promoted to wiki' }
    case 'in-review':
      return { label: 'In review', className: 'nb-review-pill', aria: 'Awaiting review' }
    case 'changes-requested':
      return { label: 'Changes', className: 'nb-review-pill', aria: 'Changes requested' }
    case 'discarded':
      return { label: 'Discarded', className: 'nb-draft-pill', aria: 'Discarded draft' }
    case 'draft':
    default:
      return { label: 'Draft', className: 'nb-draft-pill', aria: 'Draft, not yet reviewed' }
  }
}
...
export default function ByLineStrip({
  ...
  return (
    <div className="nb-byline" aria-label={aria}>
      ...
      <span className={meta.className} aria-hidden="true">{meta.label}</span>
