...
const AGENT: NotebookAgentSummary = {
  ...
  entries: [
    ...
    { entry_slug: 'e2', title: 'Entry two', last_edited_ts: new Date().toISOString(), status: 'promoted' },
    { entry_slug: 'e3', title: 'Entry three', last_edited_ts: new Date().toISOString(), status: 'in-review' },
  ],
  total: 3,
  promoted_count: 1,
...
describe('<AgentShelf>', () => {
  it('renders the agent name and role', () => {
    render(<AgentShelf agent={AGENT} onOpenAgent={() => {}} onOpenEntry={() => {}} />)
  ...
  it('invokes the appropriate callbacks when name or card is clicked', async () => {
    ...
    render(
      <AgentShelf agent={AGENT} onOpenAgent={onOpenAgent} onOpenEntry={onOpenEntry} />,
    )
  ...
  it('caps preview to 5 entries by default', () => {
    ...
    render(<AgentShelf agent={many} onOpenAgent={() => {}} onOpenEntry={() => {}} />)
