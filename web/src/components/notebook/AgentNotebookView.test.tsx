...
describe('<AgentNotebookView>', () => {
  ...
  it('renders the specified entry when entrySlug is provided', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('heading', { name: 'Pricing objections' })).toBeInTheDocument(),
    )
  })
  ...
  it('shows landing prompt when agent has no entries', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/PM has not written anything/)).toBeInTheDocument(),
    )
  })
  ...
  it('shows agent-not-found fallback when API returns null agent', async () => {
    vi.spyOn(api, 'fetchAgentEntries').mockResolvedValue({ agent: null, entries: [] })
    render(
      <AgentNotebookView
        agentSlug="zzz"
        entrySlug={null}
