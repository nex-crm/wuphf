...
function mkReview(id: string, state: ReviewItem['state'], title: string): ReviewItem {
  return {
    id,
    agent_slug: 'pm',
    entry_slug: 'e',
...
describe('<ReviewQueueKanban>', () => {
  ...
  it('optimistically moves a card when approved from the drawer', async () => {
    vi.spyOn(api, 'fetchReviews').mockResolvedValue([mkReview('r1', 'pending', 'My card')])
    ...
    await waitFor(() => expect(screen.getByTestId('nb-review-drawer')).toBeInTheDocument())
