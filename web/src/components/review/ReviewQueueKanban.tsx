...
export default function ReviewQueueKanban() {
  ...
  const active = activeId ? reviews.find((r) => r.id === activeId) ?? null : null

  const handleStateChange = async (id: string, nextState: ReviewState) => {
    // Optimistic update.
    setReviews((prev) => prev.map((r) => r.id === id ? { ...r, state: nextState } : r))
    try {
  ...
  return (
    <div className="notebook-surface" data-testid="review-queue-surface">
      <a href="#nb-review-main" className="nb-skip-link">Skip to review queue</a>
      <main id="nb-review-main" className="nb-review-queue" aria-label="Review queue">
        ...
        {loading ? (
          <div className="nb-loading" aria-busy="true">Loading reviews…</div>
        ) : error ? (
          <>
      ...
      {active && (
        <ReviewDetail
          ...
          onApprove={(id) => { void handleStateChange(id, 'approved'); setActiveId(null) }}
          onRequestChanges={(id) => { void handleStateChange(id, 'changes-requested'); setActiveId(null) }}
        />
      )}
    </div>
  )
