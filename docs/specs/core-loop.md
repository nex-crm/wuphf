...
▶ **RESUME HERE (2026-06-11 ~20:45): v3 EVAL GRADED 6/10** (v1=5, v2=5.5; artifacts .icp-eval/v3/). FIX-2 grounding DELIVERED LIVE (zero fabrication, retrieval/memory works, moat back); UI-honesty 3/3; post-done revision FIXED (3.5min). FIX-1 and FIX-3 FAILED LIVE despite 76/76 evals — **the lesson: live lifecycle paths route around the mechanisms; next lane must enumerate live control-flow paths from the v3 log and write eval checks against THOSE.** GRADER TOP-3 (round 2): (1) one truthful task state machine + working human verbs — Approve&Start terminally approved zero-work tasks (J2), human Approve/Reopen/review-RC/review-Approve all dead (200-no-op/no-POST/silent 400/500), deps must release on completion-with-artifact not approval, no terminal state without artifact on ANY path (+1.5); (2) every human utterance reaches an agent + blocking asks are loud — request-changes text STILL lost (3rd run), thread replies dead-letter, owners wake on any task-channel human message, blocking interview buried 44min wedged office (+1); (3) workspace isolation + destructive-op guard — agents executed in the HOST repo worktree, a stop-revert git checkout destroyed the deliverable; bind DoD→verification on every execution path (+1). Step 8 (skills+policies) FIRST FULLY DELIVERED step. Maya conditional-keep ("thinks like my team — steering wheel comes off"), Sam churn (3rd strike + property damage), CRO no (approval log = theater).
