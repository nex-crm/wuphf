...
▶ **RESUME HERE (2026-06-11 ~02:30): v2 ICP EVAL GRADED 5.5/10 (baseline 5/10).** Loop complete in the BACKEND (45/45 deterministic evals) but the LIVE run shows the spine bypassed: 4 of 9 loop steps BROKEN (define / grounded-context / honest-done / compounding-retrieval). Grader artifacts: `.icp-eval/v2/{observations.md,grading-report-raw.txt,shots/}`. WORST NEW FINDINGS: CEO fabricated 3 humans when retrieval missed its own wiki + ignored a live stop order; CEO self-approved over a standing human rejection; request-changes text never reaches agents (2/2); DoD never machine-encoded at intake (J3 fake-done repeats); $61k→$6k truncation again (shipped into deliverable); fake revision (zero artifact delta) passed review; entity graph empty of customers in live runs; skills compiled on disk but invisible in UI. GRADER TOP-3 FIX FAMILIES: (1) human-no sovereignty — pipe request-changes text into packets + open human rejection hard-blocks terminal transitions + Reopen symmetric with Close (+1.5, IN FLIGHT); (2) ground execution in retrieval + anti-fabrication gate + kill title-truncation context path (+1.5, QUEUED); (3) done-means-done THIRD ask — wire DoD→verification at intake, artifact-delta on resubmission, post-done message routing to owner (+1, QUEUED). Wins to keep: zero [REDACTED], instant chat-minted policies that changed artifacts, Wikipedia-grade articles w/ honesty banners, auto-compiled playbook→skill, <1min feedback re-engagement.
