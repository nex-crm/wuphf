...
| **P1b-ii** | Broker wiring (`internal/team`): route a per-task `orchestrator=langgraph\|broker` flag to `DispatchClient` instead of the in-process turn loop, write the returned projection back onto the task record, behind a flag, one task-type. Web renders via projection. | A real single-agent task completes through LangGraph and shows correctly in the web. |
