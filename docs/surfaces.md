...
| Team-member sharing | requests | `/humans`, `/humans/me`, `/humans/invites`, `/humans/invites/accept`, `/humans/sessions`, `wuphf share` | `partial`: Access & Health shows connection health; lets the host create, copy, refresh, and stop invites; and disconnects active team-member browser sessions | `partial`: `cmd/wuphf/share.go` mints the private-network invite URL; session revocation and disconnect are web-only | Host broker tokens stay out of shared browser sessions; richer sharing settings should extend the same humans contract | `cmd/wuphf/share_test.go`, `internal/team/broker_human_share_test.go`, `web/src/components/apps/HealthCheckApp.test.tsx`, `web/src/api/platform.test.ts` |
