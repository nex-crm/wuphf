# Progress: WUPHF Surfaces Development
...
- [x] Created `cli/platform-plugins/` with 7 template files:
  ...
  - windsurf-workflows: wuphf-ask.md, wuphf-remember.md, wuphf-search.md
...
- [x] PR #31 merged, published to npm as `@wuphf/wuphf@0.1.20`
...
- [x] `wuphf setup`: 3-option key regeneration (reuse email / change email / keep)
- [x] Email persisted in `~/.wuphf/config.json`
- [x] Status integrations fetch timeout: 120s -> 5s
- [x] `wuphf integrate connect <name>` — single command (gmail, slack, etc.)
...
- [x] File scanner with .wuphf.toml config
- [x] Platform detection (10+ platforms)

### v0.1.19 — Platform Rules + Setup Hierarchy (2026-03-09, folded into v0.1.20)
- [x] `wuphf setup` now installs rules + MCP for all detected platforms (hierarchy: plugins > rules > MCP)
- [x] `--with-mcp` replaced with `--no-rules` (skip rules/instruction files)
- [x] README restructured: `wuphf setup` is primary Quick Start, manual steps in collapsible sections
- [x] Root README updated: integration table shows `wuphf setup` instead of `npx @wuphf-ai/cli`
...
### P0: Interactive Integration Connection in `wuphf setup`
- [ ] During `wuphf setup`, after platform install, show available integrations (Gmail, Slack, etc.)
...
- [x] Cursor: `.cursor/rules/wuphf.md` — rules + MCP
- [x] Windsurf: `.windsurf/rules/wuphf.md` — rules + MCP
- [x] VS Code / Copilot: `.github/instructions/wuphf.instructions.md` — instructions + MCP
- [x] Cline: `.clinerules/wuphf.md` — rules + MCP
- [x] Continue.dev: `.continue/rules/wuphf.md` — rules + MCP
- [x] Zed: `.rules` (append) — rules + MCP
- [x] Kilo Code: `.kilocode/rules/wuphf.md` — rules + MCP
- [x] OpenCode: `AGENTS.md` (append) — rules + MCP
- [x] Aider: `CONVENTIONS.md` (append) — rules only (no MCP support)
- [x] `wuphf setup` installs per-platform (hierarchy: plugins > rules > MCP)
