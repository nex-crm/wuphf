...
- `SanitizedStringPolicy` (`src/sanitized-string.ts:1`) - text sanitization policy: `strip-zero-width` (default denylist), `allow-zwj` (default denylist but keeps U+200D for emoji sequences), or `allowlist` (the moat - rejects every `\p{C}` and `Default_Ignorable_Code_Point` except tab/newline/carriage return).
...
| 4 | §3.5 | `SanitizedString.fromUnknown` sparse arrays, array side properties, circular arrays/objects, and invalid policy values from untyped callers. | Object graph walking is the sanitizer's main boundary. | Add table tests for array graph hazards and a runtime-options guard decision test. |
