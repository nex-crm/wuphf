...
- **Decision Packet on-disk JSON wire shape uses camelCase, not PascalCase.** The Lane C/E pre-integration draft serialised `TaskID`, `LifecycleState`, `ParentTaskID`, etc. as PascalCase; the integrated tip writes `taskId`, `lifecycleState`, `parentTaskId` to match Lane G's frontend. There are no production users of the on-disk packet JSON yet, so no migration is needed; tools that read `~/.wuphf/tasks/<id>/decision_packet.json` directly need to use the camelCase keys going forward.
