...
- **Skills app reskinned as pixel-art Pokémon trading cards.** Every entry in the Skills tab is a Pokémon-TCG-style card with a procedurally-generated pixel-art portrait, a status-driven type palette (active = electric, proposed = psychic with a "NEEDS REVIEW" stamp, disabled = dark, archived = steel), and a 3D card flip that reveals a back face with full skill metadata (status, owners, created by, created/updated dates, source) plus a "View full SKILL.md →" CTA that opens the SidePanel preview. Front face shows the skill's title + creator byline, the procedural portrait at 144px, status/owner stat strip, the trigger as a "Triggers on" row promoted from the back so the catalog-scan use case ("which skill fires on cron?") doesn't pay a 700ms flip per card, and the description as flavor text that scrolls inside the card on overflow. The flip control sits in the same bottom slot on both faces so users develop muscle memory for the toggle. Real Pokémon TCG aspect ratio (63:88) with rounded outer corners fused with the pixel-border treatment, scanline overlay for CRT feel, and `prefers-reduced-motion` support. Press Start 2P + VT323 fonts loaded with preload. Reuses the existing `drawPixelAvatar` procedural sprite system so a skill scoped to `@ceo` always shows the CEO portrait. All in-app skill flows (approve, reject, suggest changes, archive, disable, invoke) preserved below the card. CSS `contain: paint` caps repaint cost on grids of 20+ cards. A11y: card faces use `inert` (front when flipped, back when not) so off-axis buttons leave the tab order; back-face detail list uses VT323 13px uppercase letterspaced labels; stat strip wrapped in a `<dl>` with visually-hidden Status/Owners labels for screen readers; flip button uses `aria-pressed` (toggle button semantics, not disclosure).
