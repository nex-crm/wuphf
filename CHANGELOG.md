...
- **Private-network co-founder sharing with `wuphf share`.** A running office can now mint a one-use invite, start a private Tailscale/WireGuard web listener, and let one co-founder join the shared office without exposing the local broker bearer token. Public binds are blocked by default; existing single-user `npx wuphf` launches are unchanged.
