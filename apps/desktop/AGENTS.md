...
12. **No telemetry, no analytics, no crash reporters that exfiltrate.** No `app.setAsDefaultProtocolClient` for arbitrary schemes. No `crashReporter.start({ uploadToServer: true })`. Local crash dumps and local rotated JSONL logs under Electron's `app.getPath("logs")` are fine; uploading is forbidden in this branch.
...
16. **Lefthook + CI gates are mandatory.** `bash scripts/check-ipc-allowlist.sh` runs pre-push and in CI. `bash scripts/check-invariants.sh` runs the structural deny-list. The root `desktop` CI job runs install, typecheck, lint, test, coverage, audit, IPC allowlist, invariants, and build when `apps/desktop/**`, root `package.json`, or `bun.lock` changes. The root lefthook pre-push config mirrors the desktop typecheck/test/allowlist/invariants gates for staged desktop TypeScript. Neither may be skipped (`--no-verify` is forbidden per repo CLAUDE.md). If a gate is wrong, fix the gate in the same PR — never bypass.

Dependabot proposes weekly desktop Electron stack bumps. Reviewers follow
`docs/runbooks/electron-stack-maintenance.md` before merging Electron,
electron-vite, Vite, Vitest, coverage, TypeScript, or Node type updates.
