...
The avatar system is generated from `office-sheet.png` plus role specs in
`scripts/generate-avatar-sprites.mjs`.

## Source Of Truth

- `avatarRoleSpecs` defines built-in roles, aliases, source office sprite,
...
2. Pick a `baseSpriteId` from the source sheet.
...
Generated outputs are intentionally committed for web, video, and terminal
renderers so all surfaces use the same avatar catalog.
