...
source_path: team/playbooks/attrib.md
...
1. Call `team_wiki_read` with `article_path="team/playbooks/attrib.md"` to load the canonical playbook body.
...
- The execution log at `team/playbooks/attrib.executions.jsonl` is append-only — wrong outcomes are corrected by adding a new entry, never by editing or deleting an existing one.
- This skill recompiles automatically whenever the source playbook changes. Do not edit `SKILL.md` directly; edit `team/playbooks/attrib.md` instead.

## Source

The canonical playbook lives at `team/playbooks/attrib.md`. This file is a deterministic compilation — see `internal/team/playbook_compiler.go`.
